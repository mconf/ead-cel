...
LOCAL_MODULE    := eadcel
#Instrucoes: descomentar o LOCAL_SRC_FILES desejado e comentar o LOCAL_SRC_FILES que estava ativo
#1) Teste x:
#LOCAL_SRC_FILES := x.cpp \
				    y.cpp
				   			   
#2) Teste das libs do IVA:
LOCAL_SRC_FILES := eadcel.cpp \
				   eadcelnative/opengl.cpp \
				   eadcelnative/bench.cpp \
				   eadcelnative/convert.cpp	\
				   eadcelnative/params.cpp  
...
LOCAL_LDLIBS += -llog \
				...
				$(LOCAL_PATH)/../libsIVA/libsockets.so \
				$(LOCAL_PATH)/../libsIVA/libqueue.so \
				$(LOCAL_PATH)/../libsIVA/libnet.so \
				$(LOCAL_PATH)/../libsIVA/libdecode.so \
				$(LOCAL_PATH)/../libsIVA/libencode.so			

include $(BUILD_SHARED_LIBRARY)
#fim do modulo eadecel

#inicio dos modulos do iva. Se mudar algo em um deles, compilar todos novamente,
#senao o script de compilacao nao ira copiar os arquivos .so para as pastas corretamente
#modulo thread
include $(CLEAR_VARS)
...
LOCAL_SHARED_LIBRARIES := thread common sockets
...
LOCAL_SHARED_LIBRARIES := thread common sockets queue net
...
#fim dos modulos do iva


#abaixo, os 6 modulos do ffmpeg ajustados para compilar, caso queiramos modificar alguma funcao do ffmpeg
#include $(CLEAR_VARS)
...
#LOCAL_ARM_MODE := arm
#LOCAL_CFLAGS := -DHAVE_AV_CONFIG_H
#LOCAL_SHARED_LIBRARIES := avutil avcodec swscale
#LOCAL_LDFLAGS := 
#LOCAL_SRC_FILES := \
